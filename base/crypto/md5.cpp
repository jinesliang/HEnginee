...
void MD5::MD5Transform(unsigned int state[4], unsigned char block[64])
{
	...
	for (int i = 0; i < Round_times; ++i)
	{
		...
		ROUND_OPER(F, a, b, c, d, x[x_idx], sign[round*4], base[x_idx]); 
		ROUND_OPER(F, d, a, b, c, x[x_idx+1], sign[round*4+1], base[x_idx+1]); 
		ROUND_OPER(F, c, d, a, b, x[x_idx+2], sign[round*4+2], base[x_idx+2]); 
		ROUND_OPER(F, b, c, d, a, x[x_idx+3], sign[round*4+3], base[x_idx+3]); 
	}
	/*Round 2*/
	int round = 1;
	for (int i = 0; i < Round_times; ++i)
	{
		int x_idx = round*16+4*i;
		ROUND_OPER(G, a, b, c, d, x[x_idx], sign[round*4], base[x_idx]); 
		ROUND_OPER(G, d, a, b, c, x[x_idx+1], sign[round*4+1], base[x_idx+1]); 
		ROUND_OPER(G, c, d, a, b, x[x_idx+2], sign[round*4+2], base[x_idx+2]); 
		ROUND_OPER(G, b, c, d, a, x[x_idx+3], sign[round*4+3], base[x_idx+3]); 
	}
	/*Round 3*/
	int round = 2;
	for (int i = 0; i < Round_times; ++i)
	{
		int x_idx = round*16+4*i;
		ROUND_OPER(H, a, b, c, d, x[x_idx], sign[round*4], base[x_idx]); 
		ROUND_OPER(H, d, a, b, c, x[x_idx+1], sign[round*4+1], base[x_idx+1]); 
		ROUND_OPER(H, c, d, a, b, x[x_idx+2], sign[round*4+2], base[x_idx+2]); 
		ROUND_OPER(H, b, c, d, a, x[x_idx+3], sign[round*4+3], base[x_idx+3]); 
	}
	/*Round 4*/
	int round = 3;
	for (int i = 0; i < Round_times; ++i)
	{
		int x_idx = round*16+4*i;
		ROUND_OPER(I, a, b, c, d, x[x_idx], sign[round*4], base[x_idx]); 
		ROUND_OPER(I, d, a, b, c, x[x_idx+1], sign[round*4+1], base[x_idx+1]); 
		ROUND_OPER(I, c, d, a, b, x[x_idx+2], sign[round*4+2], base[x_idx+2]); 
		ROUND_OPER(I, b, c, d, a, x[x_idx+3], sign[round*4+3], base[x_idx+3]); 
	}
