...
const char squence[]="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
...
bool Base64::encode(const char *instr, int inlen, char *outstr, int& outlen)
{
	...
	{
		int left = inlen-i;
		char idx0 = (instr[i]>>2)&0x3f;
		char idx1 = (instr[i]&0x03)<<4;
		char idx2 = 64;
		char idx3 = 64;
		if (left > 1)
		{
			idx1 |= (instr[i+1]>>4)&0x0f;
			idx2 = (instr[i+1]&0x0f)<<2;

			if (left > 2)
			{
				idx2 |= (instr[i+2]>>6)&0x03;
				idx3 = instr[i+2]&0x3f;
			}
		}
...
bool Base64::decode(const char *instr, int inlen, char *outstr, int& outlen)
{
	...
	for (int i = 0, j = 0; i < inlen; i+=4, j+=3)
	{
		...
		outstr[j] = ((idx0&0x3f)<<2)|((idx1&0x30)>>4);
		outstr[j+1] = ((idx1&0x0f)<<4)|((idx2&0x3c)>>2);
		outstr[j+2] = ((idx2&0x03)<<6)|((idx3&0x3f));
	}
...
bool Base64::reverse_encode(const std::string& instr, std::string& outstr)
{
	int outlen = (instr.length()+2)/3*4;
	char *out = new char[outlen+1];
	bool ret = reverse_encode(instr.data(), instr.length(), out, outlen);
	if (ret)
	{
		outstr = std::string(out, outlen);
	}
...
bool Base64::reverse_encode(const char *instr, int inlen, char *outstr, int& outlen)
{
	int len = (inlen+2)/3*4;
	if (outlen < len)
	{
...
bool Base64::reverse_decode(const std::string& instr, std::string& outstr)
{
	int outlen = instr.length()/4*3;
	char *out = new char[outlen+1];
	bool ret = reverse_decode(instr.data(), instr.length(), out, outlen);
	if (ret)
	{
		outstr = std::string(out, outlen);
	}
...
bool Base64::reverse_decode(const char *instr, int inlen, char *outstr, int& outlen)
{
	int len = inlen/4*3;
	if (outlen < len)
	{
